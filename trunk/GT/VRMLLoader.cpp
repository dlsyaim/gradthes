...
computeNormals( Mesh &m ) {
	...
	for(i = 0; i < (int)m.numVerts; i++)
		count[i] = 0;

	vector3 normal;

	for(i = 0; i < (int)m.numFaces; i++)//calculate running average by iterating faces
	{
...
createArrays( VRIndexedFaceSet *ifset, Mesh &m ) {
	...
	for (int j = 0; j < (int)ifset->coordIndex.numValues; j += 4) {
...
loadVRML( char *filename ) {
	...
	if (!root) {
		std::cout << "loadVRML: could not load file " << filename << std::endl;
		return meshes;
	}
	VRMFNode nodes;
	...
	for (int i = 0; i < (int)nodes.numValues; i++) {
		if (faceSets[i]->triangulate() != VR_OK) {
			std::cerr << "Unable to triangulate indexed face set " << i << " in " << filename << std::endl;
		}
